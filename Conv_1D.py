...
class ConvLayer:
    ...
    def __init__(self, kernel_size:int, weights:np.ndarray = None, bias:np.ndarray = None, in_channels:int = 1, out_channels:int = 1, stride:int = 1, dilation:int = 1, zero_padding:int = 0):
        if weights is None:
            weights = np.random.randn(out_channels, kernel_size, in_channels).astype(np.float32)
        else:
    ...
    def forward(self, input_data:np.ndarray) -> np.ndarray: 
        ...
        result_size = int(np.ceil((input_used.shape[0] - self.dilated_weights.shape[1] + 1) / self.stride))
...
if __name__ == "__main__":
    ...
    kernel = ConvLayer(kernel_size=3, weights=np.array(peso), out_channels=chanels, in_channels=chanels_in, dilation=3, stride=1)

    input_data = np.array([[0, 6, 1, 2],
                        [1, 5, 1, 4],
                        [2, 4, 1, 6],
                        [3, 3, 1, 7],
                        [4, 2, 1, 8],
